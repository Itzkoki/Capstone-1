...
const Appointment = {
   ...
   */
  async create({ intakeFormId, clientId, preferredDatetime, modality }) {
    const result = await db.query(
      `INSERT INTO appointments (intake_form_id, client_id, preferred_datetime, modality)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [intakeFormId, clientId, preferredDatetime, modality || null]
    );
  ...
  /**
   * Count active appointments for a specific date (for daily limit enforcement).
   * Active statuses: pending_review, approved, confirmed, reschedule_proposed
   */
  async countByDate(dateStr) {
    const result = await db.query(
      `SELECT COUNT(*)::int AS count
       FROM appointments
       WHERE status IN ('pending_review', 'approved', 'confirmed', 'reschedule_proposed')
         AND (
