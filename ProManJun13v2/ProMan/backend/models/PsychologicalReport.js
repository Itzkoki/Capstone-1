...
const PsychologicalReport = {
  ...
  async findArchived(restrictPsychId = null) {
    ...
    q += ` ORDER BY pr.archived_at DESC`;
    const r = await db.query(q, params);
    return r.rows;
  },

  // Soft-deleted reports (the Trash) — Clinical Director view.
  async findTrash() {
    const r = await db.query(
      `SELECT pr.*, rt.name AS template_name, rt.template_type,
              u.full_name AS psychologist_name
  ...
  // Soft delete: move to Trash by stamping deleted_at. Child rows are retained
  // so the report can be fully restored.
  async softDelete(id) {
    const r = await db.query(
      `UPDATE psychological_reports SET deleted_at = NOW(), updated_at = NOW()
       WHERE id = $1 AND deleted_at IS NULL RETURNING *`, [id]
    );
    return r.rows[0] || null;
  },

  // Bulk soft delete. If restrictPsychId is given, only that author's reports
  // are affected. Returns the ids actually moved to Trash.
  async softDeleteMany(ids, restrictPsychId = null) {
  ...
  async restore(id) {
    const r = await db.query(
      `UPDATE psychological_reports SET deleted_at = NULL, updated_at = NOW()
       WHERE id = $1 RETURNING *`, [id]
    );
    return r.rows[0] || null;
  },

  // Permanent delete (empties from Trash). Child rows cascade; audit logs detach.
  async hardDelete(id) {
    await db.query(`DELETE FROM psychological_reports WHERE id = $1`, [id]);
  },

  // --- Read / Unread state ---
  async setReadStatus(ids, isRead, restrictPsychId = null) {
    let q = `UPDATE psychological_reports SET is_read = $1, updated_at = NOW()
             WHERE id = ANY($2::int[]) AND deleted_at IS NULL`;
    const params = [isRead, ids];
    if (restrictPsychId) { q += ` AND psychologist_id = $3`; params.push(restrictPsychId); }
    q += ` RETURNING id`;
    const r = await db.query(q, params);
    return r.rows.map(x => x.id);
  },
  ...
  // --- Archive ---
  async archiveMany(ids, restrictPsychId = null) {
    let q = `UPDATE psychological_reports SET archived_at = NOW(), updated_at = NOW()
             WHERE id = ANY($1::int[]) AND deleted_at IS NULL AND archived_at IS NULL`;
    const params = [ids];
    if (restrictPsychId) { q += ` AND psychologist_id = $2`; params.push(restrictPsychId); }
    q += ` RETURNING id`;
    const r = await db.query(q, params);
  ...
  async unarchive(id, restrictPsychId = null) {
    let q = `UPDATE psychological_reports SET archived_at = NULL, updated_at = NOW() WHERE id = $1`;
    const params = [id];
    if (restrictPsychId) { q += ` AND psychologist_id = $2`; params.push(restrictPsychId); }
    q += ` RETURNING *`;
    const r = await db.query(q, params);
